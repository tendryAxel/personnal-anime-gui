...
class CachingUtilities:
    ...
    @classmethod
    def async_caching[**P, T](
        cls, func: Callable[P, Awaitable[T]]
...
def isAnime_validation(to_valid) -> TypeIs[kitsu.Anime]:
    ...
    raise TypeError(
        f"Need parsing implementation of {type(to_valid) = }, that contains: {to_valid}"
    )
...
async def _api_request[T](
    request: Callable[[kitsu.Client], CoroutineType[Any, Any, T]],
) -> T:
    client = kitsu.Client()
...
class PageParam:
    ...
    def __post_init__(self):
        if self.page_number < 0:
            raise ValueError(
                f"Page number must be positive number, not {self.page_number}"
            )
